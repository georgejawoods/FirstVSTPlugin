...
if %1 == 1 (echo Making MyNewPlugin Windows DEMO VERSION distribution ...) else (echo Making MyNewPlugin Windows FULL VERSION distribution ...)
...
REM - msbuild MyNewPlugin-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild MyNewPlugin.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild MyNewPlugin.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\MyNewPlugin.aaxplugin\Contents\Win32\MyNewPlugin.aaxplugin --out .\build-win\aax\bin\MyNewPlugin.aaxplugin\Contents\Win32\MyNewPlugin.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\MyNewPlugin.aaxplugin\Contents\x64\MyNewPlugin.aaxplugin --out .\build-win\aax\bin\MyNewPlugin.aaxplugin\Contents\x64\MyNewPlugin.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\MyNewPlugin.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\MyNewPlugin.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "MyNewPlugin Installer" ".\installer\MyNewPlugin Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\MyNewPlugin Installer.exe" ".\installer\MyNewPlugin Demo Installer.exe"
REM -del ".\installer\MyNewPlugin Installer.exe"
REM -)
